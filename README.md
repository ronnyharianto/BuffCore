...
| **BuffCore.Web.Server** | Available (0.1.0) | ASP.NET Core server conventions: versioned `BaseController`, commit/rollback `TransactionFilter<TDbContext>`, permission/API-key authorization (`PermissionAuthorizationFilter`, `PermissionAuthorizeAttribute`, `ApiPrincipal`), JWT-claims to `CurrentUserAccessor` mapping (`CurrentClaimsMapper`), and `WebApplicationBuilder` extensions for controllers, Swagger, CORS, and JWT bearer authentication. |
